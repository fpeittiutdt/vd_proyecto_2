<script>
  ...
  let valoresZodiac = ["Sagitario", "Piscis", "Aries", "Tauro", "Géminis", "Escorpio", "Virgo", "Libra", "Cáncer", "Leo", "Acuario", "Capricornio"];
  ...
  <div class="container1">
    {#each Object.entries(data_by_semester) as [month, list_data]}
      <p style="font-weight: bold; font-size: 30px">
        {month != series.length ? month + " ° cuatrimestre del año" : "No ve series"}
      </p>
  ...
  <div class="container2">
    <div class="fila-botones" style="margin-bottom: 30px;">
      ...
      {#each valoresZodiac as signoZodiac, i}
        <input class="botones-filtro" id={i} type="button" value="{signoZodiac}" on:click="{() => changeFilter(signoZodiac, i)}">
      {/each}
    </div>
    <div>
      <div class="big-circle" style="height: {initial_size}px; width: {initial_size}px;">
        {#each dataByZodiac as entry, id}
          <div class="circle-container">
            <div
              class="circle-entry"
              style="height: {circle_entry_size}px; width: {circle_entry_size}px; transform: translate({transformar(dataByZodiac.length, initial_size/2)[id]
                .x}px, {transformar(dataByZodiac.length, initial_size/2)[id].y}px); animation: none;"
            >
              {#if watch_series(entry.watch)}
